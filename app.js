...
// ThanksDiary 2.7.2-final-ui232r2
(function(){'use strict';
  // Firebase init
  ...
  document.getElementById('btnSaveQA').onclick=()=> saveDaily();
  document.getElementById('btnSaveEmotion').onclick=()=> saveDaily();
  document.getElementById('btnSaveThanks').onclick=()=> saveDaily();
  document.getElementById('btnSaveNote').onclick=()=> saveDaily();
  document.getElementById('btnSaveTags').onclick=()=> saveDaily();
  ...
  // Search
  document.getElementById('btnSearch').onclick=async()=>{ const term=(document.getElementById('searchInput').value||'').trim(); const out=document.getElementById('searchResults'); out.innerHTML=''; if(!term) return; if(!auth.currentUser) return out.innerHTML='<li class="hint">로그인 후 검색 가능합니다.</li>'; const daily=await userDoc().collection('daily').orderBy('updatedAt','desc').limit(500).get(); let hits=0; daily.docs.forEach(d=>{ const v=d.data(); const hay=(v.answer||'')+' '+(v.dailyNote||'')+' '+(v.evEvent||'')+' '+(v.evThought||'')+' '+(v.evEmotion||'')+' '+(v.evResult||' ')+' '+(v.tags||[]).join(' '); if(hay.includes(term)){ const li=document.createElement('li'); li.innerHTML='<strong>'+ (v.date||d.id) +'</strong><div>'+ (v.dailyNote||v.answer||'').slice(0,120) +'</div>'; out.append(li); hits++; } }); if(!hits) out.innerHTML='<li class="hint">결과 없음</li>'; };
