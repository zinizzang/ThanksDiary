...
// ----- State & Storage -----
const storeKey = 'jinijjang-v1-3-1';
const settingsKey = 'jinijjang-settings-v1-3-1'; // {darkMode, lockEnabled, pinHash, credId}
...
function applyTheme(){
  ...
  const useDark = !!s.darkMode;
  document.documentElement.classList.toggle('dark', useDark);
  if(darkModeChk) darkModeChk.checked = useDark;
}
if(darkModeChk){
  darkModeChk.addEventListener('change', ()=>{
    const s = loadSettings();
    s.darkMode = darkModeChk.checked;
...
function setActiveTab(id){
  document.querySelectorAll('.tab-btn').forEach(a=>a.classList.toggle('active', a.getAttribute('aria-controls')===id));
  document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active', p.id===id));
}
function router(){
  const h = location.hash.replace('#/','');
  const id = pages.includes(h) ? h : 'daily';
  setActiveTab(id);
}
...
clearDaily.addEventListener('click', ()=>{
  ...
  saveAll(data);
  loadDaily();
  savedStatus.textContent = '삭제됨';
});

// init daily date at load
...
function renderMissions(items){
  ...
  items.forEach((m, idx)=>{
    ...
    del.addEventListener('click', ()=>{ if(!confirm('삭제할까요?')) return; items.splice(idx,1); renderMissions(items); saveWeeklyData(); });
...
saveWeekly.addEventListener('click', ()=>{ saveWeeklyData(); alert('주간 저장 완료'); });
clearWeekly.addEventListener('click', ()=>{
  if(!confirm('이 주차 데이터를 모두 지울까요?')) return;
  const data = loadAll(); const key = currentWeekKey();
  delete data.weekly[key]; saveAll(data); loadWeekly(); alert('삭제됨');
});
...
randomHealing.addEventListener('click', ()=>{
  ...
  healingText.value = pick;
});
startCopy.addEventListener('click', ()=>{
  copyArea.classList.toggle('hidden');
  if(!copyArea.classList.contains('hidden')){
    copyPad.value = healingText.value || '';
    copyPad.focus();
  }
});
// init week
...
function doSearch(){
  ...
  const qn = q.replace(/^#/,'').toLowerCase();
...
document.getElementById('exportJSON').addEventListener('click', ()=>{
  ...
  a.href = url; a.download = `jinijjang_backup_${ymd(new Date())}.json`; a.click();
...
document.getElementById('exportCSV').addEventListener('click', ()=>{
  ...
  function downloadZip(JSZip){
    ...
    zip.generateAsync({type:"blob"}).then((content)=>{
      ...
      const a = document.createElement('a');
      a.href = url; a.download = `jinijjang_csv_${ymd(new Date())}.zip`; a.click();
      URL.revokeObjectURL(url);
    });
  }
...
document.getElementById('importJSON').addEventListener('click', ()=>{
  ...
  reader.onload = (e)=>{
    ...
    }catch(err){ alert('가져오기 실패: JSON 확인'); }
  };
  reader.readAsText(f);
});

// ----- Lock (disabled by default; can enable in Settings) -----
  ...
  try{
    ...
    if(!s.credId){
      ...
      const cred = await navigator.credentials.create({
        publicKey: {
          challenge,
          rp: {name: 'Jinijjang Journal', id: location.hostname},
    ...
    await navigator.credentials.get({
      publicKey: { challenge, allowCredentials: [{id: credIdBytes, type:'public-key', transports:['internal']}], userVerification:'required', timeout:60000 }
    });
    lockScreen.classList.add('hidden');
  }catch(e){ alert('인증 실패/취소: '+(e.message||e)); }
});
function applyLockUI(){
  const s = loadSettings();
  lockEnabledChk.checked = !!s.lockEnabled;
  lockScreen.classList.toggle('hidden', !s.lockEnabled);
  lockScreen.setAttribute('aria-hidden', (!s.lockEnabled).toString());
}
