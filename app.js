...
// v2.3.2 compat – Firebase compat SDK, no service worker, hard routing, working modal
(function(){
  // Firebase config (사용자 제공 값)
  ...
  // ---- Local storage ----
  const storeKey='td-v232-data', settingsKey='td-v232-settings';
  ...
  clearWeekly.addEventListener('click', async ()=>{ if(!confirm('이 주차의 주간 데이터를 모두 지울까요?')) return; const data=loadAll(); const key=currentWeekKey(); delete data.weekly[key]; saveAll(data); const user=auth.currentUser; if(user){ await db.collection('users').doc(user.uid).collection('weekly').doc(key).delete().catch(()=>{}); } loadWeekly(); });
  ...
  $('#refreshCache').addEventListener('click', ()=>{ alert('이 버전은 서비스워커를 사용하지 않아요. 새로고침만 하면 최신이 적용됩니다.'); });
  ...
  miniLogout?.addEventListener('click', async ()=>{ await auth.signOut(); });
  ...
  modalSignIn?.addEventListener('click', async ()=>{
    try{ await auth.signInWithEmailAndPassword(loginEmail.value.trim(), loginPwd.value); closeLoginModal(); }
    catch(e){ loginMsg.textContent = '로그인 실패: ' + (e.message||e); }
  });
  modalSignUp?.addEventListener('click', async ()=>{
    try{ await auth.createUserWithEmailAndPassword(loginEmail.value.trim(), loginPwd.value); closeLoginModal(); }
  ...
  // defer로 로드되므로 DOMContentLoaded 시점과 유사
  init();
})();
